...
// A server of this test's own: its own port, its own scratch folder, and its own rations. The harness leaves the
// environment out (FLY_APP_NAME and FRAME_TRUST_PROXY above all), so each of these says for itself where it runs.
function start(env = {}) {
  const data = makeData('frame-hard-');
  writeHousehold(data, 'alpha', {name: 'Alpha'});
  return startServer({data, env: {FRAME_AUTH: 'required', ...env}});
}
